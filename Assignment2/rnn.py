...
class RNN(nn.Module):
    def __init__(self, input_dim, h, numOfLayer):  # Add relevant parameters
...
if __name__ == "__main__":
    ...
    # valid_data_vectorized = convert_to_vector_representation(valid_data, word2index)
    
    model = RNN(50, args.hidden_dim,1)  # Fill in parameters
    ...
    while not stopping_condition:
        ...
        for minibatch_index in tqdm(range(N // minibatch_size)):
            ...
            for example_index in range(minibatch_size):
                ...
                vectors = [word_embedding[i.lower()] if i.lower() in word_embedding.keys() else word_embedding['unk'] for i in input_words ] # use custom embedding
                
                # Transform the input into required shape
                numpy_array=np.stack(vectors)
                #vectors = torch.tensor(vectors).view(len(vectors), 1, -1)
                vectors=torch.tensor(numpy_array)
    ...
    for input_words, gold_label in tqdm(test_data):
        ...
        total_test += 1
        # print(predicted_label, gold_label)
    print("Test accuracy: {}".format(correct_test / total_test))
